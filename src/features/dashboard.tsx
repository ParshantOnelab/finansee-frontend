...
function Dashboard() {
    ...
    const [topInsightsData, setTopInsightsData] = useState({
    ...
    useEffect(() => {
        if (isSuccess && topInsights) {
            setTopInsightsData({
                top_products: topInsights.top_products.map((item: { product_name: string; customer_count: string }) => {
                    return {
                        name: item.product_name,
                        value: Number(item.customer_count.slice(0, -1))
                    }
                }) || topProducts,
                top_biases: topInsights.top_biases.map((item: { bias: string; customer_count: string }) => {
                    return {
                        name: item.bias,
                        value: Number(item.customer_count.slice(0, -1))
                    }
        ...
        , [isSuccess, topInsights]);

    const navigate = useNavigate();

    return (
        <div className='py-6 bg-slate-100 dark:bg-gray-900 overflow-auto'>
            <div className='w-full max-w-[85%] sm:max-w-[100%] md:max-w-[100%]  mx-auto px-2 sm:px-4 md:px-6 lg:px-8'>
